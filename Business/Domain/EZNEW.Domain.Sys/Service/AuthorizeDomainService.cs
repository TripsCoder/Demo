...
namespace EZNEW.Domain.Sys.Service
{
    ...
    /// 授权服务
    /// </summary>
    public static class AuthorizeDomainService
    {
        ...
        public static Result ModifyUserAuthorize(IEnumerable<UserAuthorize> userAuthorizes)
        {
            ...
            IEnumerable<Authority> roleAuthoritys = AuthorityDomainService.GetAuthorityList(authQuery);
        ...
        public static bool Authentication(Authentication auth)
        {
            ...
            User nowUser = UserDomainService.GetUser(auth.User.SysNo);//当前用户
            if (nowUser == null)
            {
                return false;
            }
            ...
            AuthorityOperation nowOperation = AuthorityOperationDomainService.GetAuthorityOperation(auth.Operation.ControllerCode, auth.Operation.ActionCode);//授权操作信息
